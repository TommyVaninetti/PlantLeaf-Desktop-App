...
class WideComboBox(QComboBox):
    """
    A QComboBox whose drop-down is wide enough to read.

    Qt lays the popup out to the *combo's* width at the moment it is shown, and
    elides anything longer — so widening the view once at construction time has no
    effect (it gets overwritten). The width has to be forced from inside
    showPopup(), immediately before the popup is laid out.
